...
echo -n "The entered URL was: ${_REGURL}" >> ~/Desktop/checkvt.log
...
then
	...
	echo -n "The entered URL did not contain http or https." >> ~/Desktop/checkvt.log
	echo -n "http has been added, url is now: ${_REGURL}" >> ~/Desktop/checkvt.log
fi
...
if [[ -z "${_CHECKURL}" ]]
then
	echo -n "(no redirect): the checked url does not contain any redirect." >> ~/Desktop/checkvt.log
	...
	then
		...
		_REGURL+='/'
		echo -n "The entered URL did not contain a / at end, so it was added." >> ~/Desktop/checkvt.log
		echo -n "The url is now ${_REGURL}" >> ~/Desktop/checkvt.log
	fi
	...
	echo -n "The non-redirected final URL was: ${_FINALURL}" >> ~/Desktop/checkvt.log
	open "${_FINALURL}"

else
	echo -n "(redirect found): the checked url variable contains a redirect to: ${_CHECKURL}" >> ~/Desktop/checkvt.log
	echo -n "The new redirected URL is now ${_CHECKURL}" >> ~/Desktop/checkvt.log
	...
	do
		...
		then
			...
			echo -n "no more redirects found" >> ~/Desktop/checkvt.log
		else
			_CHECKURL="${_REDIRECTURL}"
			echo -n "(redirect found): the URL contains a redirect to ${_REDIRECTURL}" >> ~/Desktop/checkvt.log
		fi
	...
	then
		...
		echo -n "The last checked URL did not contain http or https." >> ~/Desktop/checkvt.log
		echo -n "http has been added, URL is now: ${_LASTCHECKURL}" >> ~/Desktop/checkvt.log
	fi
	...
	then
		...
		echo -n "Fixed, redirect URL is now: ${_LASTCHECKURL}" >> ~/Desktop/checkvt.log
	fi
	...
	# then we open the combined final URL
	echo -n "The redirected final URL was: ${_FINALURL}" >> ~/Desktop/checkvt.log
	open "${_FINALURL}"
fi
